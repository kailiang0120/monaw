...
type ViewMode = 'runs' | 'errors' | 'backend' | 'legacy'
...
export function ObservabilityPanel() {
  ...
  const [legacyRuns, setLegacyRuns] = useState<EvaluationRun[]>([])
  const [legacyReplay, setLegacyReplay] = useState<EvaluationReplayResult | null>(null)
  ...
  const loadLegacyRuns = async () => {
    setLegacyRuns(await fetchEvaluationRuns(50))
  }
  ...
  const refreshAll = async () => {
    ...
    setError('')
    try {
      await Promise.all([loadSummary(), loadRuns(), loadErrors(), loadBackendLog(), loadLegacyRuns()])
      if (selectedRunId) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (view === 'legacy') await loadLegacyRuns()
      } catch (exc) {
  ...
  const replayLegacyRun = async (runId: string) => {
    setActionRunId(runId)
    setLegacyReplay(null)
    setNotice('')
    setError('')
    try {
      const result = await replayEvaluationRun(runId)
      setLegacyReplay(result)
      setNotice(`Legacy replay created ${result.conversation_id} with status ${result.status}.`)
      await loadLegacyRuns()
    } catch (exc) {
      setError(exc instanceof Error ? exc.message : 'Legacy replay failed')
    } finally {
      setActionRunId('')
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['runs', 'errors', 'backend', 'legacy'] as ViewMode[]).map((item) => (
          <button
            key={item}
            type="button"
          ...
          >
            {item === 'runs' ? 'Runs' : item === 'errors' ? 'Errors' : item === 'backend' ? 'Backend log' : 'Legacy evals'}
          </button>
        ))}
      ...
      {view === 'legacy' && (
        <LegacyEvaluationsView
          runs={legacyRuns}
          loading={loading}
          replayingRunId={actionRunId}
          lastReplay={legacyReplay}
          onReplay={(runId) => void replayLegacyRun(runId)}
        />
      )}
    </div>
  )
...
function LegacyEvaluationsView({
  runs,
  loading,
  replayingRunId,
  lastReplay,
  ...
  lastReplay: EvaluationReplayResult | null
  onReplay: (runId: string) => void
}) {
  if (runs.length === 0 && !loading) {
    return (
      <div className="panel-muted rounded-xl px-4 py-10 text-center text-sm text-neutral-500">
        No legacy trajectory runs found.
      </div>
    )
  }
  return (
    <div className="space-y-3">
      {lastReplay && (
        <div className="rounded-lg border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-xs text-emerald-200">
          Legacy replay created {lastReplay.conversation_id} with status {lastReplay.status}.
        </div>
      )}
      <div className="rounded-xl border border-white/[0.07]">
        <div className="grid grid-cols-[84px_1fr_96px_96px] border-b border-white/[0.07] bg-black/10 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-neutral-600">
          <span>Status</span>
          <span>Message</span>
          <span>Tools</span>
          <span>Replay</span>
        ...
        <div className="max-h-[520px] overflow-auto">
          {runs.map((run) => (
            <div
              key={run.run_id}
              className="grid grid-cols-[84px_1fr_96px_96px] gap-2 border-b border-white/[0.06] px-3 py-2 text-xs"
            >
              <span className={`status-pill w-fit ${run.success ? statusClass('complete') : statusClass('paused')}`}>
                {run.success ? 'success' : 'failed'}
              </span>
              <span className="min-w-0">
                <span className="block truncate text-neutral-200">{run.last_user_message || '(empty message)'}</span>
                <span className="block truncate text-[11px] text-neutral-600">
                  {shortId(run.run_id)} · {run.provider || '-'} / {run.model || '-'} · {formatTime(run.finished_at || run.started_at)}
                </span>
                {run.error && <span className="block truncate text-[11px] text-amber-300">{run.error}</span>}
              </span>
              <span className="text-neutral-500">{formatNumber(run.tool_count)}</span>
              <button
                type="button"
                onClick={() => onReplay(run.run_id)}
                disabled={!!replayingRunId}
                className="ghost-button h-8 w-fit rounded-lg px-2.5 text-xs disabled:opacity-50"
              >
                <Play size={13} className={replayingRunId === run.run_id ? 'animate-pulse' : ''} />
                Replay
              </button>
            </div>
          ))}
